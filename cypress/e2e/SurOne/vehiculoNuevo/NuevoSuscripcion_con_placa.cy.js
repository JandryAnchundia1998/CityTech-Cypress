...
it.only("Debe validar el flujo para vehículo usado con placa válida y cédula válida + accesorios + contratanteAsegurado", () => {
  ...
  cy.get('#subsQuoteYes').click()
    ...
    cy.get('.col-12.mt-3 > .input-group > .form-floating > .form-control').type(dato.vehiculo.placa);

    cy.get(':nth-child(5) > .col-12 > .container-ngselect-icon > .ng-select > .ng-select-container').type(dato.uso).click();
    ...
    if (
      ...
      cy.get('.custom-textbox > .form-control').type(accesorios.valor_1); // Ingresar el valor del primer accesorio
